...
final class ParametersEncodedEndpointConfiguration: QuickConfiguration {
    override static func configure(_ configuration: QCKConfiguration) {
        sharedExamples("endpoint with encoded parameters") { (context: SharedExampleContext) in
            let parameters = context()["parameters"] as! Parameters
...
@MainActor final class EndpointSpec: QuickSpec {
    nonisolated private class var simpleGitHubEndpoint: Endpoint {
    ...
    override class func spec() {
        var endpoint: Endpoint! = Self.simpleGitHubEndpoint
        ...
        describe("unsuccessful converting to urlRequest") {
            ...
            context("when parameter encoding is unsuccessful") {
                it("throws a .parameterEncoding error") {
                    ...
                    struct InvalidParameter: Sendable {}
        ...
        describe("given endpoint comparison") {
            context("when task is .uploadMultipart") {
                it("should correctly acknowledge as equal for the same url, headers and form data") {
                    endpoint = endpoint.replacing(task:
                            .uploadMultipartFormData([MultipartFormBodyPart(provider: .data("test".data(using: .utf8)!), name: "test")]))
                    let endpointToCompare = endpoint.replacing(task:
                            .uploadMultipartFormData([MultipartFormBodyPart(provider: .data("test".data(using: .utf8)!), name: "test")]))
                ...
                it("should correctly acknowledge as not equal for the same url, headers and different form data") {
                    endpoint = endpoint.replacing(task:
                            .uploadMultipartFormData([MultipartFormBodyPart(provider: .data("test".data(using: .utf8)!), name: "test")]))
                    let endpointToCompare = endpoint.replacing(task:
                            .uploadMultipartFormData([MultipartFormBodyPart(provider: .data("test1".data(using: .utf8)!), name: "test")]))
                    expect(endpoint) != endpointToCompare
                }
            }
            ...
            context("when task is .uploadCompositeMultipart") {
                it("should correctly acknowledge as equal for the same url, headers and form data") {
                    endpoint = endpoint.replacing(task:
                            .uploadCompositeMultipartFormData([MultipartFormBodyPart(provider: .data("test".data(using: .utf8)!), name: "test")], urlParameters: [:]))
                    let endpointToCompare = endpoint.replacing(task: .uploadCompositeMultipartFormData([MultipartFormBodyPart(provider: .data("test".data(using: .utf8)!), name: "test")], urlParameters: [:]))

                    expect(endpoint) == endpointToCompare
                }

                it("should correctly acknowledge as not equal for the same url, headers and different form data") {
                    endpoint = endpoint.replacing(task: .uploadCompositeMultipartFormData([MultipartFormBodyPart(provider: .data("test".data(using: .utf8)!), name: "test")], urlParameters: [:]))
                    let endpointToCompare = endpoint.replacing(task: .uploadCompositeMultipartFormData([MultipartFormBodyPart(provider: .data("test1".data(using: .utf8)!), name: "test")], urlParameters: [:]))
