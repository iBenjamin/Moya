...
nonisolated(unsafe) let failureEndpointClosure = { (target: GitHub) -> Endpoint in
...
enum HTTPBin: TargetType, AccessTokenAuthorizable {
    ...
    case uploadMultipartFormData(MultipartFormData, Parameters?)
    case uploadMultipartBodyParts([MultipartFormBodyPart], Parameters?)
    case validatedUploadMultipartFormData(MultipartFormData, Parameters?, [Int])
    case validatedUploadMultipartBodyParts([MultipartFormBodyPart], Parameters?, [Int])
    ...
    var task: Task {
        ...
        case .uploadMultipartBodyParts(let bodyParts, let urlParameters), .validatedUploadMultipartBodyParts(let bodyParts, let urlParameters, _):
            if let urlParameters = urlParameters {
                return .uploadCompositeMultipartFormData(.init(parts: bodyParts), urlParameters: urlParameters)
            } else {
                return .uploadMultipartFormData(.init(parts: bodyParts))
            }
        }
    }
